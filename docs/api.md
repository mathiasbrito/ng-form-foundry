...
| `initiallyExpanded` | `boolean` | `false` | Whether the tree starts fully expanded (every node with children unfolded) when it binds to a schema/form pair, as if the expand-all control had been pressed. Read at bind time — it sets the initial state; later collapses/expands and the expand-all toggle take over, and a structural rebuild keeps whatever is currently open. |
